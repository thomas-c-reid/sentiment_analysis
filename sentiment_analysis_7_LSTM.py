...
x = np.array(vectorize_layer(df['review']))
y = df.iloc[:, 1]
...
# training dataset then gets split into training and validation splits
x_train, x_val, y_train, y_val = train_test_split(x_train, y_train, test_size=0.2, random_state=random_state)


history = model.fit(x_train, y_train, epochs=num_of_epochs, validation_data=(x_val, y_val), callbacks=callbacks)

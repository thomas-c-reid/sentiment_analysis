...
def objective(trial):
    params = {
        ...
        "max_iter": trial.suggest_int("max_iter", 100, 1000),  # Maximum number of iterations for optimization
    }
    ...
    for train_idx, val_idx in CV.split(x_train, y_train):
        train_x, train_y = x[train_idx], y.iloc[train_idx]
        val_x, val_y = x[val_idx], y.iloc[val_idx]
        ...
        acc = accuracy_score(val_y, binary_predictions)

        average_acc.append(acc)
    return np.mean(average_acc)
